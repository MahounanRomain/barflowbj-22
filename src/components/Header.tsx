...
const Header: React.FC<HeaderProps> = ({
  ...
  return <header className={`bg-background/95 backdrop-blur-lg border-b border-border/60 px-4 py-3 flex items-center justify-between sticky top-0 z-40 glass-effect animate-slide-up shadow-lg ${!isMobile ? 'border-l-4 border-l-primary' : ''}`}>
      <div className="flex items-center space-x-3 animate-slide-in-left">
        {(!isOnDashboard || !isMobile) && <Button variant="ghost" size="sm" onClick={() => navigate(-1)} className="h-8 w-8 p-0 hover-lift transition-smooth focus-ring hover:bg-primary/10">
            <ArrowLeft size={18} className="text-primary" />
          </Button>}
        <div className="flex flex-col">
          <h1 className="text-lg font-semibold text-foreground transition-smooth">
            {isOnDashboard && !title && isMobile ? <span className="bg-gradient-to-r from-primary via-accent to-primary bg-clip-text text-transparent animate-gradient-shift">BarFlow</span> : getPageTitle()}
          </h1>
          {isOnDashboard && !title && isMobile && <p className="text-[10px] text-muted-foreground font-light -mt-0.5 animate-fade-in">
      ...
      <div className="flex items-center space-x-2 animate-slide-in-right">
        ...
        {rightContent}
        {isAuthenticated && isOnDashboard && <UserAccountDialog />}
      </div>
    </header>;
};
