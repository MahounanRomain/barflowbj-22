  ...
  return <A11yProvider>
      <div className="flex min-h-screen w-full bg-background">
        ...
        <div className={`flex-1 flex flex-col min-h-screen ${!isMobile && !isNotFoundPage ? 'ml-0' : 'w-full'}`}>
          ...
          {/* Page Content */}
          <main className={`flex-1 bg-gradient-to-br from-background via-bar-light/20 to-primary/5 ${isMobile && !isNotFoundPage ? "pb-24" : ""} ${!isMobile ? 'min-h-[calc(100vh)]' : 'min-h-[calc(100vh-5rem)]'}`} id="main-content" tabIndex={-1}>
            <ErrorBoundary>
              {children}
            </ErrorBoundary>
          </main>
